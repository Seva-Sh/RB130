...
class TodoListTest < Minitest::Test
  ...
  def test_find_by_title
    str1 = @todo1.title
    str2 = "Hello"
    assert_equal(@todo1, @list.find_by_title(str1))
    assert_nil(@list.find_by_title(str2))
  end

  def test_all_done
    @todo1.done!
    list = @list.all_done
    assert_equal([@todo1], list.todos)
  end

  def test_all_not_done
    list = @list.all_not_done
    assert_equal([@todo1, @todo2, @todo3], list.todos)
  end
  ...
  def test_mark_done
    ...
    @list.mark_done(str1)
    assert_equal([@todo2], @list.all_done.todos)
  end

  def test_mark_all_done
    @list.mark_all_done
    assert_equal([@todo1, @todo2, @todo3], @list.all_done.todos)
  end

  def test_mark_all_undone
    @list.mark_all_done
    @list.mark_all_undone
    assert_equal([], @list.all_done.todos)
  end
  ...
  def test_select
    ...
    refute_equal(list.to_s, @list.select{ |todo| todo.done? }.to_s)
  end
end
